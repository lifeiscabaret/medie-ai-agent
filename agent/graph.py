...
class MedicationData(BaseModel):
    ...
    timestamp: str = Field(description="데이터 측정 시간 (KST)")
...
async def save_medication_log(user_id: str, taken_at: str, pill_name: str, source: str = "MANUAL"):
    ...
    # TODO: POST /medication-logs
    pass

async def update_user_profile(user_id: str, data: dict):
    """TODO: 조원 API 연결"""
    print(f"[DB] 프로필 업데이트: {user_id} / {data}")
    # TODO: PATCH /users/{user_id}/profile
    pass
...
def navigate_node(state: AgentState):
    ...
    try:
        ...
        return {**state, "response_text": "화면 이동할게요! 멍!", "action_required": "NAVIGATE", "next_step": "HOME", "show_confirmation": False, "params": {}}
...
def complete_dose_node(state: AgentState):
    ...
    # TODO: DB 저장 (조원 API 연결 후 활성화)
    # await save_medication_log(state["user_id"], taken_at, "복용약")
...
반드시 params에 time과 pillId를 포함하세요.
"""),
    ...
    try:
        ...
        return {**state, "response_text": "알람 시간을 말씀해주세요! 멍!", "action_required": "NONE", "next_step": "NONE", "show_confirmation": False, "params": {}}
...
def iot_action_node(state: AgentState):
    ...
    return {
        **state,
        "response_text": f"약통에서 움직임이 감지됐어요! 방금 약 드셨나요? 멍멍!",
        "action_required": "SHOW_CONFIRMATION",
        "next_step": "NONE",
        "show_confirmation": True,
        "params": {"weight_change": weight_change, "detected_at": iot_data.get("timestamp", "")}
    }
...
def send_to_joone_fastapi(state: AgentState):
    ...
    try:
        print(f"[Maddy] 조원 서버로 전송 중... ({JOONE_API_URL})")
